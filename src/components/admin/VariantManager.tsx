...
const TYPE_PRESETS: Record<string, { sizes: string[] }> = {
  ...
  beauty: {
    sizes: ["5ml", "10ml", "30ml", "50ml", "100ml", "200ml", "500ml"],
  },
  fragrance: { sizes: ["1ml", "2ml", "5ml", "10ml", "30ml", "50ml", "100ml"] },
  electronics: {
    sizes: ["8GB/256GB", "16GB/512GB", "32GB/1TB", "Standard", "Pro"],
  },
...
export default function VariantManager({
  ...
  const presets =
    TYPE_PRESETS[productType as keyof typeof TYPE_PRESETS] ||
    TYPE_PRESETS.clothing;

  // Sync internal selection state with incoming variants on mount or if variants change outside
  useEffect(() => {
    const sizes = Array.from(
      new Set(variants.map((v) => v.size).filter((s) => s && s !== "Default"))
    );
  ...
  const addManualVariant = () => {
    onChange([
      ...variants,
      { size: "", color: hasColor ? "" : "Default", stock: 0, sku: "" },
    ]);
  };
    ...
    <div className="space-y-6">
      <div className="space-y-4">
        ...
        <div className="space-y-3 p-5 bg-slate-50 dark:bg-gray-800/50 rounded-2xl border border-slate-100 dark:border-gray-700">
          ...
          <div className="flex gap-2 max-w-sm pt-2">
            <Input
              placeholder={`Or custom ${sizeLabel}...`}
              value={customSize}
      ...
      {hasColor && (
        <div className="space-y-3 p-5 bg-slate-50 dark:bg-gray-800/50 rounded-2xl border border-slate-100 dark:border-gray-700">
                ...
                <div
                  ...
                  style={{
                    background: color.hex.includes("gradient")
                      ? color.hex
                      : undefined,
                    backgroundColor: color.hex.includes("gradient")
                      ? undefined
                      : color.hex,
                  }}
                />
          ...
          <div className="flex gap-2 max-w-sm pt-2">
            <Input
              placeholder={`Or custom ${colorLabel}...`}
              value={customColor}
      ...
      <div className="pt-6 border-t border-slate-100 dark:border-gray-700">
        <div className="flex justify-between items-center mb-6 gap-4">
          ...
          <div className="flex gap-3">
            ...
            >
              <Plus className="w-3.5 h-3.5 mr-2" /> Manual Add
            </Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {variants.map((v, i) => (
            <div
              key={i}
              className="flex flex-col p-3 bg-white border rounded-xl shadow-sm hover:shadow-md transition-shadow group space-y-3"
            >
              ...
              <div className="grid grid-cols-2 gap-2 mt-2">
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    value={v.stock}
                    onChange={(e) =>
                      updateVariantStock(i, parseInt(e.target.value) || 0)
                    }
                  />
                </div>
                {variantPricing && (
        ...
        {variants.length === 0 && (
          <div className="text-center py-12 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
            ...
            <p className="text-sm text-gray-500">
              No variants generated yet. Toggle sizes and colors above.
            </p>
          </div>
        )}
      </div>
