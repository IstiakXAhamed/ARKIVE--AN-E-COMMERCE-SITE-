  ...
  const fetchUsers = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Parse permissions if they are JSON strings (Arkive stores them as String? @db.LongText)
        const parsedUsers = fetchedUsers.map((u: any) => ({
             ...u,
    ...
    try {
      const res = await fetch('/api/admin/super-admin/permissions', {
        ...
        body: JSON.stringify({
          ...
          permissions: userPermissions, // Backend should handle JSON stringification
          password: password,
        })
      })
