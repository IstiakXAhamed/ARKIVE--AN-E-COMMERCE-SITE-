...
import { toast } from 'sonner' // Changed to sonner as per Arkive usage (assumed, verifying later if needed, mostly used in modern shadcn) or keep useToast

// Assuming Arkive uses standard Shadcn toast
...
const FEATURE_TOGGLES: FeatureToggle[] = [
  ...
  // Feature Visibility (Arkive specific)
  { id: 'showFreeShipping', label: 'Show Free Shipping', description: 'Display free shipping badge', field: 'showFreeShipping', icon: Truck, category: 'marketplace' },
  { id: 'showEasyReturns', label: 'Show Easy Returns', description: 'Display easy returns badge', field: 'showEasyReturns', icon: Truck, category: 'marketplace' },
  { id: 'showCOD', label: 'Show COD Badge', description: 'Display COD available badge', field: 'showCOD', icon: CreditCard, category: 'marketplace' },
  { id: 'showAuthentic', label: 'Show Authentic Badge', description: 'Display 100% authentic badge', field: 'showAuthentic', icon: Sparkles, category: 'marketplace' },
]
...
export function FeatureToggles() {
    ...
    try {
      ...
      if (data.settings) {
        ...
        FEATURE_TOGGLES.forEach(f => {
          mapped[f.field] = settings[f.field] ?? true
        })
        
        // Handle Spin Wheel Config if exists (it's JSON string in DB)
        if (settings.spinWheelConfig) {
           try {
             const config = JSON.parse(settings.spinWheelConfig)
             mapped['spinWheelEnabled'] = config.enabled ?? true
           } catch (e) {
             mapped['spinWheelEnabled'] = true
           }
        }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Handle SpinWheel special case
      if ('spinWheelEnabled' in payload) {
         // fetch current config to preserve other fields
         const res = await fetch('/api/admin/settings')
         const data = await res.json()
         let currentConfig = { enabled: true, cooldownMinutes: 1440, delaySeconds: 10, prizes: [] }
         if (data.spinWheelConfig) {
            try {
               currentConfig = JSON.parse(data.spinWheelConfig)
            } catch (e) {}
         }
         
         const newConfig = { ...currentConfig, enabled: payload.spinWheelEnabled }
         payload.spinWheelConfig = JSON.stringify(newConfig)
  ...
  const getCategory = (cat: string) => {
    const cats: Record<string, { label: string; color: string }> = {
      marketplace: { label: 'Storefront', color: 'bg-blue-100 text-blue-700' },
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>Enable or disable global site features</CardDescription>
          </div>
