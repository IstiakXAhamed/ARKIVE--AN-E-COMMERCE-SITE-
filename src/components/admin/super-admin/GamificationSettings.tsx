...
export function GamificationSettings() {
  ...
  const fetchConfig = async () => {
    try {
      ...
      if (data.settings && data.settings.spinWheelConfig) {
        try {
          const parsed = JSON.parse(data.settings.spinWheelConfig)
          setConfig(parsed)
        } catch (e) {
          console.error("Failed to parse wheel config", e)
        }
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch('/api/admin/settings', {
        method: 'PUT', // Use PUT for Arkive's settings API
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          spinWheelConfig: JSON.stringify(config) // Send as JSON string
        })
      })
